...
static inline int change_pte_read_status(pte_t* pte, bool can_read) {
	if (!pte) { return 0; }
	if (can_read) {
		/* ARM64: set PTE_USER + Access Flag to grant user read */
		set_pte(pte, set_pte_bit(*pte, __pgprot(PTE_USER | PTE_AF)));
	} else {
		/* ARM64: clear PTE_USER (AF) + PTE_WRITE to revoke user read */
		pte_t new_pte = __pte(pte_val(*pte) & ~(PTE_USER | PTE_WRITE));
		set_pte(pte, new_pte);
	}
	return 1;
}
