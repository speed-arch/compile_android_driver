...
static ssize_t OnCmdHideKernelModule(struct ioctl_request *hdr, char __user* buf) {
	...
	if (g_rwProcMem_devp->is_hidden_module == false) {
		...
		/*
		 * Follow kernel free_module() pattern:
		 *  1) Mark UNFORMED -> m_show() in procfs.c skips us
		 *  2) list_del_rcu -> safe against concurrent RCU readers
		 *  3) kobject_del -> remove /sys/module/<name>/
		 *  4) synchronize_rcu -> wait for readers that started before removal
		 */
		...
		/* 5. Wipe module name in memory -> forensic obfuscation */
		memset(__this_module.name, 0, sizeof(__this_module.name));

#ifdef CONFIG_KALLSYMS
		/* 6. Wipe kallsyms -> hides module symbols from /proc/kallsyms */
